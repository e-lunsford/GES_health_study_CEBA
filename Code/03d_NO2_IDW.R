#-------------------------------------------------------------------------------
# title: "03d NO2 Inverse Distance Weighting"  
# author: E Lunsford  
# date: 2025-08-07 
#  
...
#################################################################################
# Load EPA NO2 data from "02" file.
#################################################################################

load(file = "Outputs/denver_no2_2017_2021.RData") # Loads as no2_aqs2
...
ggplot() +
  ...
  geom_sf(data = den_no2, aes(col = no2_ppb))
...
no2_nbhd_idw_ppb <- idw(formula = no2_ppb ~ 1,
                        locations = den_no2,
                        newdata = sf_nbhd_centroids,
                        idp = 2.0)

# 3. Spatially join the neighborhood names with the IDW prediction values.
IDW_no2_nbhd_level_ppb <- sf_nbhd_centroids %>%
  st_join(no2_nbhd_idw_ppb) %>%
  dplyr::rename(no2_pred_ppb = var1.pred)
...
ggplot() +
  geom_sf(data = den_nbhd_wgs84,
          ...
          colour = "red",
          linewidth = 0.5) +
  geom_point(data = IDW_no2_nbhd_level_ppb,
             mapping = aes(color = no2_pred_ppb,
                           #size = 1.0,
                           geometry = geometry),
             stat = "sf_coordinates") +
...
# 2. Use the IDW formula to predict concentrations (ppb) at the census tract centroids.
no2_census_tract_idw_ppb <- idw(formula = no2_ppb ~ 1,
                                locations = den_no2,
                                newdata = sf_census_tract_centroids,
                                idp = 2.0)

# 3. Spatially join the census tract information with the IDW prediction values.
IDW_no2_census_tract_level_ppb <- sf_census_tract_centroids %>%
  st_join(no2_census_tract_idw_ppb) %>%
  dplyr::rename(no2_pred_ppb = var1.pred)

# 4. Map it.
ggplot() +
  geom_sf(data = den_tract_nbhd,
          inherit.aes = F,
          fill = NA,
          colour = "red",
          linewidth = 0.5) +
  geom_point(data = IDW_no2_census_tract_level_ppb,
             mapping = aes(color = no2_pred_ppb,
                           #size = 1.0,
                           geometry = geometry),
             stat = "sf_coordinates") +
...
no2_idw <- idw(no2_ppb ~ 1, 
               den_no2_sp,
               newdata = spgrdWithin,
               idp=2.0)

# C2. Set IDW as raster.
raster_no2_idw <- raster(no2_idw)

# C3. Get raster values.
val1 = as.numeric(c(minValue(raster_no2_idw):maxValue(raster_no2_idw)))

# C4. Clip to CCoD.
## Crop function
crop_raster_no2_idw <- crop(raster_no2_idw, den_tract_sp)

## Mask function
mask_raster_no2_idw <- mask(raster_no2_idw, den_tract_sp)
...
## cropped
crop_raster_no2_idw_df <- as.data.frame(crop_raster_no2_idw, xy=TRUE)

## masked
mask_raster_no2_idw_df <- as.data.frame(mask_raster_no2_idw, xy = TRUE) %>%
  na.omit()

# D2. Map IDW raster with CCoD census tracts.
## cropped
ggplot() +
  geom_raster(data = crop_raster_no2_idw_df,
...
## masked
ggplot() +
  geom_raster(data = mask_raster_no2_idw_df,
              aes(x = x, y = y, fill = var1.pred)) +
  scale_fill_viridis_c()+
  geom_sf(data = den_ct_nbhd,
          inherit.aes = F,
...
rasValue_nbhd <- terra::extract(raster_no2_idw, sp_nbhd_centroid)

# E1 Step 3. Combine raster values to Method 1.
IDW_no2_nbhd_centroids <- cbind(IDW_no2_nbhd_level_ppb, rasValue_nbhd)

IDW_no2_nbhd_centroids <- IDW_no2_nbhd_centroids %>%
  dplyr::rename(no2_ras_ppb = rasValue_nbhd)
...
ggplot() +
  geom_sf(data = den_ct_nbhd,
          ...
          colour = "red",
          linewidth = 0.5) +
  geom_point(data = IDW_no2_nbhd_centroids,
             mapping = aes(color = no2_ras_ppb,
                           #size = 1.0,
                           geometry = geometry),
             stat = "sf_coordinates") +
  scale_color_viridis_c(option = "C")
...
rasValue_census_tract <- terra::extract(raster_no2_idw, sp_den_tract_centroid)

# E2 Step 3. Combine raster values to Method 2.
IDW_no2_census_tract_centroids <- cbind(IDW_no2_census_tract_level_ppb, 
                                        rasValue_census_tract)

IDW_no2_census_tract_centroids <- IDW_no2_census_tract_centroids %>%
  dplyr::rename(no2_ras_ppb = rasValue_census_tract)
...
ggplot() +
  geom_sf(data = den_ct_nbhd,
          ...
          colour = "red",
          linewidth = 0.5) +
  geom_point(data = IDW_no2_census_tract_centroids,
             mapping = aes(color = no2_ras_ppb,
                           #size = 1.0,
                           geometry = geometry),
             stat = "sf_coordinates") +
...
# Save both datasets.
save(file = "Outputs/IDW_NO2_nbhd_centroids_08_2025.RData",
     x = IDW_no2_nbhd_centroids)

save(file = "Outputs/IDW_NO2_ct_centroids_08_2025.RData", 
     x = IDW_no2_census_tract_centroids)
