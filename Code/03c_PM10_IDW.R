#-------------------------------------------------------------------------------
# title: "03c PM10 Inverse Distance Weighting"  
# author: E Lunsford  
# date: 2025-08-07 
#  
# This code is to spatially model PM10 daily measurements over a 5 year period
...
#################################################################################
# Load EPA PM10 data from "02" file.
#################################################################################

load(file = "Outputs/denver_pm10_2017_2021.RData")
...
dplyr::glimpse(den_pm10)

#################################################################################
# Filter Data.                                                                  #
## Filter to the pollutant standard of choice.                                  #
## Note that the PM2.5 standard was updated from 12 ug/m3 to 9 ug/m3 in 2024.   #
...
knitr::kable(sum_stats, digits = 4)
...
ggplot() +
  ...
  geom_sf(data = den_pm10, aes(col = pm10_ugm3))
...
pm10_nbhd_idw <- idw(formula = pm10_ugm3 ~ 1,
                     locations = den_pm10,
                     newdata = sf_nbhd_centroids,
                     idp = 2.0)

# 3. Spatially join neighborhood names with IDW prediction values.
IDW_pm10_nbhd_level <- sf_nbhd_centroids %>%
  st_join(pm10_nbhd_idw) %>%
  dplyr::rename(pm10_pred_ugm3 = var1.pred)
...
ggplot() +
  geom_sf(data = den_nbhd_wgs84,
          ...
          colour = "black",
          linewidth = 0.5) +
  geom_point(data = IDW_pm10_nbhd_level,
             mapping = aes(color = pm10_pred_ugm3,
                           #size = 1.0,
                           geometry = geometry),
             stat = "sf_coordinates") +
...
# 2. Use IDW formula to predict concentrations at census tract centroids.
pm10_census_tract_idw <- idw(formula = pm10_ugm3 ~ 1,
                             locations = den_pm10,
                             newdata = sf_census_tract_centroids,
                             idp = 2.0)

# 3. Spatially join census tract data with IDW prediction values.
IDW_pm10_census_tract_level <- sf_census_tract_centroids %>%
  st_join(pm10_census_tract_idw) %>%
  dplyr::rename(pm10_pred_ugm3 = var1.pred)
...
ggplot() +
  geom_sf(data = den_tract_wgs84,
          ...
          colour = "red",
          linewidth = 0.5) +
  geom_point(data = IDW_pm10_census_tract_level,
             mapping = aes(color = pm10_pred_ugm3,
                           #size = 1.0,
                           geometry = geometry),
             stat = "sf_coordinates") +
...
den_pm10_sp <- as_Spatial(den_pm10)
...
pm10_idw <- idw(pm10_ugm3 ~ 1, 
                den_pm10_sp,
                newdata = spgrdWithin,
                idp=2.0)

# C2. Set IDW as raster.
raster_pm10_idw <- raster(pm10_idw)

# C3. Get raster values.
val1 = as.numeric(c(minValue(raster_pm10_idw):maxValue(raster_pm10_idw)))

# C4. Clip to CCoD.
## Crop function
crop_raster_pm10_idw <- crop(raster_pm10_idw, den_tract_sp)

## Mask function
mask_raster_pm10_idw <- mask(raster_pm10_idw, den_tract_sp)
...
## cropped
crop_raster_pm10_idw_df <- as.data.frame(crop_raster_pm10_idw, xy=TRUE)

## masked
mask_raster_pm10_idw_df <- as.data.frame(mask_raster_pm10_idw, xy = TRUE) %>%
  na.omit()

# D2. Map IDW raster with CCoD census tracts.
## cropped
ggplot() +
  geom_raster(data = crop_raster_pm10_idw_df,
...
## masked
ggplot() +
  geom_raster(data = mask_raster_pm10_idw_df,
              aes(x = x, y = y, fill = var1.pred)) +
  scale_fill_viridis_c()+
  geom_sf(data = den_ct_nbhd,
          inherit.aes = F,
...
rasValue_nbhd <- terra::extract(raster_pm10_idw, sp_nbhd_centroid)

# E1 Step 3. Combine raster values to Method 1.
IDW_pm10_nbhd_centroids <- cbind(IDW_pm10_nbhd_level, rasValue_nbhd)

IDW_pm10_nbhd_centroids <- IDW_pm10_nbhd_centroids %>%
  dplyr::rename(pm10_ras_ugm3 = rasValue_nbhd)
...
ggplot() +
  geom_sf(data = den_ct_nbhd,
          ...
          colour = "red",
          linewidth = 0.5) +
  geom_point(data = IDW_pm10_nbhd_centroids,
             mapping = aes(color = pm10_ras_ugm3,
                           #size = 1.0,
                           geometry = geometry),
             stat = "sf_coordinates") +
  scale_color_viridis_c(option = "C")
...
rasValue_census_tract <- terra::extract(raster_pm10_idw, 
                                        sp_den_tract_centroid)

# E2 Step 3. Combine raster values to Method 2.
IDW_pm10_census_tract_centroids <- cbind(IDW_pm10_census_tract_level, 
                                         rasValue_census_tract)

IDW_pm10_census_tract_centroids <- IDW_pm10_census_tract_centroids %>%
  dplyr::rename(pm10_ras_ugm3 = rasValue_census_tract)
...
ggplot() +
  geom_sf(data = den_ct_nbhd,
          ...
          colour = "red",
          linewidth = 0.5) +
  geom_point(data = IDW_pm10_census_tract_centroids,
             mapping = aes(color = pm10_ras_ugm3,
                           #size = 1.0,
                           geometry = geometry),
             stat = "sf_coordinates") +
...
save(file = "Outputs/IDW_PM10_nbhd_centroids_08_2025.RData",
     x = IDW_pm10_nbhd_centroids)

save(file = "Outputs/IDW_PM10_ct_centroids_08_2025.RData",
     x = IDW_pm10_census_tract_centroids)
