#-------------------------------------------------------------------------------
# title: "03f CO Inverse Distance Weighting"  
# author: E Lunsford  
# date: 2025-08-07 
#  
# This code is to spatially model NO2 daily measurements over a 5 year period
...
#################################################################################
# Load EPA carbon monoxide data from "02" file.
#################################################################################

load(file = "Outputs/denver_co_2017_2021.RData") # loads as co_aqs2
...
ggplot() +
  geom_sf(data = metro_tract_wgs84) +
  geom_sf(data = den_nbhd_wgs84, fill = "green", col = "black") +
  geom_sf(data = den_co, aes(col = co_ppm))
...
# 2. Use IDW to predict concentrations (ppm) at neighborhood centroids.
co_nbhd_idw_ppm <- idw(formula = co_ppm ~ 1,
                       locations = den_co,
                       newdata = sf_nbhd_centroids,
                       idp = 2.0)

# 3. Spatially join the neighborhood names with the IDW prediction values.
IDW_co_nbhd_level_ppm <- sf_nbhd_centroids %>%
  st_join(co_nbhd_idw_ppm) %>%
  dplyr::rename(co_pred_ppm = var1.pred)
...
ggplot() +
  geom_sf(data = den_nbhd_wgs84,
          ...
          colour = "red",
          linewidth = 0.5) +
  geom_point(data = IDW_co_nbhd_level_ppm,
             mapping = aes(color = co_pred_ppm,
                           #size = 1.0,
                           geometry = geometry),
             stat = "sf_coordinates") +
...
# 2. Use IDW to predict concentrations (ppm) at census tract centroids.
co_census_tract_idw_ppm <- idw(formula = co_ppm ~ 1,
                               locations = den_co,
                               newdata = sf_census_tract_centroids,
                               idp = 2.0)

# 3. Spatially join census tract information with IDW prediction values.
IDW_co_census_tract_level_ppm <- sf_census_tract_centroids %>%
  st_join(co_census_tract_idw_ppm) %>%
  dplyr::rename(co_pred_ppm = var1.pred)
...
ggplot() +
  geom_sf(data = den_ct_nbhd,
          ...
          colour = "red",
          linewidth = 0.5) +
  geom_point(data = IDW_co_census_tract_level_ppm,
             mapping = aes(color = co_pred_ppm,
                           #size = 1.0,
                           geometry = geometry),
             stat = "sf_coordinates") +
...
co_idw <- idw(formula = co_ppm ~ 1, 
              locations = den_co_sp,
              newdata = spgrdWithin,
              idp = 2.0)

# C2. Set IDW as raster.
raster_co_idw <- raster(co_idw)

# C3. Get raster values.
val1 = as.numeric(c(minValue(raster_co_idw):maxValue(raster_co_idw)))

# C4. Clip to CCoD.
## Crop function
crop_raster_co_idw <- crop(raster_co_idw, den_tract_sp)

## Mask function
mask_raster_co_idw <- mask(raster_co_idw, den_tract_sp)
...
## cropped
crop_raster_co_idw_df <- as.data.frame(crop_raster_co_idw, xy=TRUE)

## masked
mask_raster_co_idw_df <- as.data.frame(mask_raster_co_idw, xy = TRUE) %>%
  na.omit()

# D2. Map IDW raster with CCoD census tracts.
## cropped
ggplot() +
  geom_raster(data = crop_raster_co_idw_df,
...
## masked
ggplot() +
  geom_raster(data = mask_raster_co_idw_df,
              aes(x = x, y = y, fill = var1.pred)) +
  scale_fill_viridis_c()+
  geom_sf(data = den_ct_nbhd,
          inherit.aes = F,
...
rasValue_nbhd <- terra::extract(raster_co_idw, sp_nbhd_centroid)

# E1 Step 3. Combine raster values to Method 1.
IDW_co_nbhd_centroids <- cbind(IDW_co_nbhd_level_ppm, rasValue_nbhd)

IDW_co_nbhd_centroids <- IDW_co_nbhd_centroids %>%
  dplyr::rename(co_ras_ppm = rasValue_nbhd)
...
ggplot() +
  geom_sf(data = den_nbhd_wgs84,
          ...
          colour = "red",
          linewidth = 0.5) +
  geom_point(data = IDW_co_nbhd_centroids,
             mapping = aes(color = co_ras_ppm,
                           #size = 1.0,
                           geometry = geometry),
             stat = "sf_coordinates") +
  scale_color_viridis_c(option = "C")
...
rasValue_census_tract <- terra::extract(raster_co_idw, sp_den_tract_centroid)

# E2 Step 3. Combine raster values to Method 2.
IDW_co_census_tract_centroids <- cbind(IDW_co_census_tract_level_ppm, 
                                       rasValue_census_tract)

IDW_co_census_tract_centroids <- IDW_co_census_tract_centroids %>%
  dplyr::rename(co_ras_ppm = rasValue_census_tract)
...
ggplot() +
  geom_sf(data = den_ct_nbhd,
          ...
          colour = "red",
          linewidth = 0.5) +
  geom_point(data = IDW_co_census_tract_centroids,
             mapping = aes(color = co_ras_ppm,
                           #size = 1.0,
                           geometry = geometry),
             stat = "sf_coordinates") +
...
# Save both datasets.
save(file = "Outputs/IDW_CO_nbhd_centroids_08_2025.RData",
     x = IDW_co_nbhd_centroids)

save(file = "Outputs/IDW_CO_ct_centroids_08_2025.RData", 
     x = IDW_co_census_tract_centroids)
